...
\begin{defi}[Enligt \cite{Lang}]
	Två vektorer $\bfx, ~\bfx'\in\hil$ är \emph{ortogonala} om $\inner{\bfx}{\bfx'}_\hil=0$. Dessutom är vektorerna \emph{ortonormala} ifall de är både ortogonala och normaliserade det vill säga $\left\|\bfx\right\|_\hil=\left\|\bfx'\right\|_\hil=1$.
\end{defi}
\begin{ex}
	Låt $\bfx=\left[1,0\right]^\intercal$ och $\bfx'=\left[0,1\right]^\intercal$. Då är $\inner{\bfx}{\bfx'}_2=\bfx^\intercal\bfx'=\left[1,0\right]\left[0,1\right]^\intercal = 1\cdot0 + 0\cdot1=0$ och vektorerna är vinkelräta. Normen av vektorerna är också 1 så de är dessutom ortogonala.
\end{ex}
...
För nästa sats behövs några konventioner för hyperplan $L=\sephyp$ i relation till en punkt $\bfy$ i $\hil$:
\begin{itemize}
	...
	\item Med det signerade avståndet från vektorn $\bfy$ till hyperplanet $L$, $\operatorname{d}^\pm\left(\bfy,L\right)$, menar man $\inf_{\bfx\in L} \left\|\bfy-\bfx\right\|_\hil$ om $f\left(\bfy\right)\geq0$ och $-\left(\inf_{\bfx\in L} \left\|\bfy-\bfx\right\|_\hil\right)$ om $f\left(\bfy\right)\leq0$.
...
\begin{thm}[Enligt \cite{ESL}]\label{thm:hyperplan}
	...
	\begin{enumerate}
		\item $\bfbeta$ är en ortogonal till alla vektorer i $L$ och kan \emph{ortonormeras} (göras ortonormal) genom
		\begin{equation*}
			\widehat{\bfbeta} = \frac{\bfbeta}{\left\|\bfbeta\right\|_\hil}.
		\end{equation*}
		\item $\inner{\bfx_0}{\bfbeta}_\hil = -\beta_0$ för alla $\mathbf{x}_0$ i $L$.
		\item
		Det signerade avståndet från en punkt $\mathbf{x}$ till hyperplanet $L$, $\operatorname{d}^\pm \left(\mathbf{x}, L\right)$, ges av
		\begin{align*}
			\inner{\mathbf{x}-\mathbf{x}_0}{\widehat{\bfbeta}}_\hil &= \frac{1}{\left\|\bfbeta\right\|_\hil}\left(\inner{\mathbf{x}}{\bfbeta}_\hil+\beta_0\right)\\
\intertext{och, ifall $\hil=\mathbb{R}^p$ med den vanliga inreprodukten,}
			\inner{\mathbf{x}-\mathbf{x}_0}{\widehat{\bfbeta}}_p &= \frac{1}{\left\|f^\prime\left(\mathbf{x}\right)\right\|_p}f\left(\mathbf{x}\right)
		\end{align*}
...
\begin{enumerate}
	...
	\end{align*}
	alltså uppfyller $\bfbeta$ kravet för normalvektorer och $\widehat{\bfbeta}:=\frac{\bfbeta}{\left\|\bfbeta
\right\|_p}$ är den normaliserade normalvektorn till hyperplanet $L$. \hfill\qedsymbol
	...
	Låt $\mathbf{x}_0$ vara en punkt i hyperplanet $L$. Då är avståndet från hyperplanet till punkten $\mathbf{x}$ lika med längden av projektionen av vektorn $\left(\mathbf{x}-\mathbf{x}_0\right)$ på hyperplanets normaliserade normal $\widehat{\bfbeta}$, $\operatorname{comp_{\widehat{\bfbeta}}} \left( \mathbf{x} - \mathbf{x}_0 \right)$. Vi får alltså att
	\begin{align*}
		\operatorname{d^\pm} \left( \mathbf{x}, L \right) &= \operatorname{comp_{\widehat{\bfbeta}}} \left( \mathbf{x} - \mathbf{x}_0 \right)
		=\underline{\underline{ \inner{\mathbf{x} - \mathbf{x}_0}{\widehat{\bfbeta}}_\hil}}\\
		&= \frac{1}{\left\|\bfbeta
\right\|_p}\left(\inner{\bfx}{\bfbeta}_\hil - \inner{\bfx_0}{\bfbeta}_\hil\right)=\underline{\underline{\frac{1}{\left\|\bfbeta
\right\|_p}\left(\inner{\bfx}{\bfbeta}_\hil + \beta_0\right)}}
	\end{align*}
	där det sista steget följer från egenskap \ref{eq:egenskap2} då $\mathbf{x}_0$ är en punkt i $L$. Om man vidare noterar att $f\left(\mathbf{x}\right)=\inner{\bfx}{\bfbeta}_\hil+\beta_0$ och $f^\prime\left(\mathbf{x}\right)=\bfbeta$ så fås även att
	\begin{equation*}
		\frac{1}{\left\|\bfbeta
\right\|_p}\left(\inner{\bfx}{\bfbeta}_\hil + \beta_0\right)=\frac{1}{\left\|f^\prime\left(\mathbf{x}\right)
\right\|_p}f\left(\mathbf{x}\right).
	\end{equation*}
	\qedhere
\end{enumerate}
\end{proof}
...
\right\|_p} + \frac{\beta_0}{\left\|\bfbeta
\right\|_p}=0\right\}$. Om $\mathbf{x}_0$ då tillhör $L_1$ så tillhör $\mathbf{x}_0$ även $L_3$ eftersom att $h\left(\mathbf{x}\right) = \frac{f\left(\mathbf{x}\right)}{\left\|\bfbeta
\right\|_p}=0$. Notera även att $\frac{1}{\left\|\bfbeta
\right\|_p}$ kunde ha varit vilket reellt tal som helst.
\end{reas}

\begin{rem}
	För att få entydiga hyperplan för klassificering kan man lägga till villkor. Om man kräver att $\left\|\bfbeta
\right\|_p=1$ och $y_i\left(\inner{\bfx_i}{\bfbeta}_\hil + \beta_0\right)\geq0$ för alla $i=1,\dots,~N$, där $y_i$ är klasserna i klassificeringsproblemet, så får man en entydig definition av hyperplanet där vektorn $\bfbeta$ ''pekar mot`` klassen där $y_i=1$ och $\beta_0$ anger det signerade avståndet (i relation till riktningen på $\bfbeta$) från origo till hyperplanet.
\end{rem}
\begin{reas}
...
\end{thm}
\begin{proof}
	Ifall ett klassificeringsproblem är linjärt separabelt så ligger alla observationer $y_i$ på rätt sida av hyperplanet definierat genom $\inner{\bfx}{\bfbeta}_\hil + \beta_0$; eller så ligger alla observationer på fel sida av hyperplanet. Vilket betyder att ifall $y_i=1$ så är $\inner{\bfx_i}{\bfbeta}_\hil + \beta_0 > 0$ och om $y_i=-1$ så är $\inner{\bfx_i}{\bfbeta}_\hil + \beta_0 < 0$. Detta betyder att $y_i\left(\inner{\bfx_i}{\bfbeta}_\hil + \beta_0\right) > 0$. Ifall $\inner{\bfx_i}{\bfbeta}_\hil + \beta_0 = 0$ är problemet inte linjärt separabelt.
\end{proof}
...
\end{aligned}
\end{equation*}
Därefter görs ännu en konvexitetsbevarande kvadratisk transformering av objektfunktionen $\left\|\bfbeta
\right\|_p$ (objektfunktionen kallas även \textit{kostfunktionen}), det vill säga man noterar att
...
\end{equation*}
Vidare kan (\ref{eq:derivprimal}) skrivas om som derivatan med avseende på hela $\bfbeta$ eftersom att $ \left[ D_{ \bfbeta }\left(L_p\right) \right]_j = D_{\left[ \bfbeta \right]_j}\left(L_p\right) $. Efter att man tar i beaktande kraven att $ D_{ \bfbeta }\left(L_p\right) = \mathbf{0} $ och $ D_{\beta_0}\left(L_p\right)=0 $ fås följande krav:
...
Kvar finns också möjligheten att $\lambda^*_i = 0$ och $y_i\left( \inner{\bfx_i}{\bfbeta^*}_p + \beta^*_0\right) = 1$.
...
\end{equation*}
Varje funktion $f\left(\bfx\right)$ är alltså en linjär kombination av funktionerna $\Phi_{\bfx_i}\left(\bfx\right)$ där $\bfx_i,~i=1,~\dots,~N$. %TODO	Klumpigt formulerat
