...
\end{equation*}
Varje funktion $f(\bfx)$ är alltså en linjär kombination av funktionerna $\Phi_{\bfx_i}(\bfx)$ där $\bfx_i,~i=1,~\dots,~N$.
...
\end{equation}
Definitionen för $\langle f, g \rangle_k$ innehåller samma koefficienter som de linjära kombinationer som definierar $f$ och $g$ men eftersom att
...
\begin{enumerate}
	...
	= \sum_{i=1}^{N}\sum_{j=1}^{N}\alpha_i\alpha_j\left\langle\phi(\bfx_i),\phi(\bfx_j)\right\rangle_P\\
	&= \sum_{i=1}^{N}\alpha_i\sum_{j=1}^{N}\alpha_j\left\langle\phi(\bfx_j),\phi(\bfx_i)\right\rangle_P
	= \sum_{i=1}^{N}\alpha_i\left\langle\sum_{j=1}^{N}\alpha_j\phi(\bfx_j),\phi(\bfx_i)\right\rangle_P\\
	&= \sum_{i=1}^{N}\alpha_i\left\langle\phi(\bfx_i),\sum_{j=1}^{N}\alpha_j\phi(\bfx_j)\right\rangle_P
	= \left\langle\sum_{i=1}^{N}\alpha_i\phi(\bfx_i),\sum_{j=1}^{N}\alpha_j\phi(\bfx_j)\right\rangle_P\\
	&= \left\|\sum_{i=1}^{N}\alpha_i\phi(\bfx_i)\right\|_P^2\geq 0
	\end{align*}
	För att visa att $\langle f, f\rangle = 0$ om och endast om $f=0$ det vill säga alla $\alpha_i, i=1,~\dots,~N$ krävs lite mera matematiskt maskineri, man behöver en motsats till Cauchy-Schwartz olikheten för kärnor det vill säga
	\begin{thm}[Cauchy-Scwartz olikhet för kärnor \cite{LearningKernels}]
		För en \emph{positiv definit} kärna $k$ (se \ref{def:positivdefinit}) och $\bfx_1,~\bfx_2\in V$ där $V$ är något vektorrum gäller att
		\begin{equation}
	...
	\begin{proof}
		...
		\begin{gather*}
			...
			\overset{\mathbf{K}_{21}=\mathbf{K}_{12}\text{ enligt (\ref{eq:expansion2})}}{\Leftrightarrow}\\
			\mathbf{K}_{11} \mathbf{K}_{22} - \mathbf{K}_{12} \mathbf{K}_{12} \geq 0\\
			\overset{a^2 = |a|^2\text{ om } a\in\mathbb{R}}{\Leftrightarrow}\\
			\mathbf{K}_{11} \mathbf{K}_{22} - \left|\mathbf{K}_{12}\right|^2 \geq 0\\
			\overset{\text{byt tillbaka } \mathbf{K}_{ij}\text{ till } k(\bfx_i, \bfx_j)}{\Leftrightarrow}\\
...
\begin{equation}
\langle f, f \rangle_k = \sum_{i=1}^{N}\sum_{j=1}^{N}\alpha_i\alpha_jk(\bfx_i,\bfx_j) = \boldsymbol{\alpha}^\intercal \mathbf{K} \boldsymbol{\alpha}
\end{equation}
där $\boldsymbol{\alpha}$ är en vektor med elementen $\alpha_1,~\alpha_2,~\dots,~\alpha_N$ och $\mathbf{K}$ är matrisen med elementen $\mathbf{K}_{ij}=k(\bfx_i,\bfx_j)$
