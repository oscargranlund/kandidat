...
L_P=\frac{1}{2}\|\bfbeta\|^2 - \sum_{i=1}^{N} \lambda_i[y_i (\mathbf{x}_i^\intercal\bfbeta + \beta_1)-1]
\end{equation}
...
\end{equation*}
Vidare kan relationen \ref{eq:derivprimal} skrivas om som derivatan med avseende på hela $\bfbeta$ eftersom att $ \left[ D_{ \bfbeta }(L_p) \right]_j = D_{\left[ \bfbeta \right]_j}(L_p) $. Efter att man tar i beaktande kraven att $ D_{ \bfbeta }(L_p) = \mathbf{0} $ och $ D_{\beta_1}(L_p)=0 $ fås följande:
\begin{align*}
