...
\begin{aligned}
	& \underset{\bfbeta,\beta_0,\|\bfbeta\|=1}{\text{max}} & & C\\
	& \text{så att} & & y_i(\mathbf{x}_i^\intercal\bfbeta+\beta_0)\geq C,\quad i=1,\dots,N
\end{aligned}
\end{equation}
där $C$ kallas marginalen och betecknar avståndet från hyperplanet till de närmaste punkterna.
\begin{rem}
	Ifall alla punkter är rätt klassificerade så anger $y_i(\mathbf{x}^\intercal_i\bfbeta+\beta_0)$ det absoluta avståndet mellan hyperplanet och punkten $\mathbf{x}_i$.
\end{rem}
...
För att se att optimeringsproblemet (\ref{opt:optimalmargin1}) har är \textit{konvext}, det vill säga har en unik lösning, måst vi skriva om det något. Vi börjar med att gör oss av med kravet att $\|\bfbeta\|=1$ genom att byta ut kraven
\begin{equation*}
y_i(\mathbf{x}_i^\intercal\bfbeta+\beta_0)\geq C,\quad i=1,\dots,N
\end{equation*}
mot kraven
\begin{equation*}
\frac{1}{\|\bfbeta\|}y_i(\mathbf{x}_i^\intercal\bfbeta+\beta_0)\geq C,\quad i=1,\dots,N
\end{equation*}
eller ekvivalent
\begin{equation*}
