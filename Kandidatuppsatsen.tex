...
\begin{enumerate}
	...
	\end{align*}
	För att visa att $\langle f, f\rangle = 0$ om och endast om $f=0$ det vill säga alla $\alpha_i, i=1,~\dots,~N$ krävs en motsats till Cauchy-Schwartz olikheten men formulerad för kärnor det vill säga
	...
	\begin{proof}
		...
		\end{equation*}
		Då $\mathbf{K}_ij = k(\bfx_i,\bfx_j)$ är symmetrisk enligt (\ref{eq:expansion2}) så fås att
		\begin{equation*}
			\mathbf{K}_{11} \mathbf{K}_{22} - \left|\mathbf{K}_{12}\right|^2
		\end{equation*}
		alltså gäller
		\begin{equation*}
	...
	\begin{equation*}
		\langle \Phi_{\bfx_k}, f\rangle_k = \langle k(\cdot, \bfx_i), f\rangle_k = \sum_{j=1}^{N}\sum_{i=1}^{N}\beta_j\alpha_i k(\bfx_j, \bfx_i)\rangle_k
	\end{equation*}
	där $\beta_j = 0$ om $j\neq k$ och $1$ om $j=k$, det vill säga
	\begin{equation}\label{eq:reproducing1}
		\langle \Phi_{\bfx_k}, f\rangle_k = \sum_{i=1}^{N}\alpha_i k(\bfx_k, \bfx_i)\rangle_k = f(\bfx_k).
	\end{equation}
	och speciellt fås att
	\begin{equation*}
		\langle \Phi_{\bfx_k}, \Phi_{\bfx_l} \rangle:k = \sum_{i=1}^{N}\sum_{j=1}^{N}\alpha_i\beta_jk(\bfx_i,\bfx_j)
	\end{equation*}
