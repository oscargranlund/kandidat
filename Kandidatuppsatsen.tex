...
\chapter{Lite Hilbertrumteori}\label{chap:hilbert}
...
\end{enumerate}
Eftersom att alla villkor i \ref{def:inreprodukt} är uppfyllda så är $\langle f, g\rangle_k = \sum_{i=1}^{N}\sum_{j=1}^{N}\alpha_i\beta_jk\left(\bfx_i,\bfx_j\right)$ där $k\left(\bfx, \bfy\right)=\left(\langle \bfx, \bfy\rangle_1 + 1\right)^2= \left\langle \phi\left(\bfx\right), \phi\left(\bfy\right)\right\rangle_3$ och $\phi\left(\bfx\right)=\left[\bfx^2, \sqrt{2}\bfx, 1 \right]^\intercal$ en inreprodukt. Om man dessutom kräver att alla Cauchyföljder konvergerar till en funktion i rummet så har man ett Hilbertrum vilket betyder att allting i kapitel \ref{chap:hilbert} stämmer även här.
...
\begin{cor}
	Om dessutom alla Cauchyföljder konvergerar till en funktion i samma rum, med avseende på normen inducerad av inreprodukten $\langle f, g\rangle_k$, så är rummet ett Hilbertrum.
\end{cor}
Betrakta funktionen $\Phi_{\bfx_l}(\bfx):=k\left(\bfx, \bfx_l\right)=\sum_{j=1}^{N}\beta_jk\left(\bfx, \bfx_l\right)$ med $\beta_j=1$ om $j=l$, 0 annars och $\bfx_l$ är fixerat.
	\begin{equation*}
	...
	\begin{equation}\label{eq:reproducing1}
		\langle \Phi_{\bfx_k}, f\rangle_k = \sum_{i=1}^{N}\alpha_i k\left(\bfx_l, \bfx_i\right) = f\left(\bfx_l\right).
	\end{equation}
	och speciellt fås att
	\begin{equation*}
	...
	\begin{equation*}
	\langle \Phi_{\bfx_l}, \Phi_{\bfx_h} \rangle_k = \alpha_l\beta_hk\left(\bfx_l,\bfx_h\right) = k\left(\bfx_l, \bfx_h\right).
	\end{equation*}
...
\begin{ex}
	...
	\begin{align*}
		...
		&= \inner{\phi\left(\bfx_i\right)}{\phi\left(\bfx_j\right)}_{\mathcal{l}^2}
	\end{align*}
	där $\phi\left(\bfx\right)=\left[\frac{e^{-\left(\frac{\bfx}{\sqrt{2}\sigma}\right)^2} \bfx^0}{\sqrt{0!}}, \frac{e^{-\left(\frac{\bfx}{\sqrt{2}\sigma}\right)^2} \bfx^1}{\sqrt{1!}}, \frac{e^{-\left(\frac{\bfx}{\sqrt{2}\sigma}\right)^2} \bfx^2}{\sqrt{2!}},\dots\right]^\intercal$.
	
	Implikationen är att motsvarande olinjära transformation skulle ge ett oändligtdimensionellt rum att jobba med om man gör transformationen direkt medan man genom kärnfunktionen $k$ kan implicit jobba i ett oändligtdimensionellt rum, något som inte hade varit möjligt utan kärnfunktionen.
\end{ex}
