...
\end{equation*}
men man måste dessutom kräva att normen $\left\|f\right\|_{\mathcal{L}^2} = \left(\int_{a}^{b}f\left(x\right)^2dx\right)^{\frac{1}{2}}$ är ändlig för alla funktioner i vektorrummet.
...
\begin{thm}\label{thm:hyperplan}
	...
	\begin{enumerate}
		...
		\begin{equation*}%TODO	Definiera normalvektor
			\widehat{\bfbeta} = \frac{\bfbeta}{\left\|\bfbeta\right\|_p}.
		\end{equation*}
		...
		\begin{equation*}
		\begin{aligned}
			\inner{\mathbf{x}-\mathbf{x}_0}{\widehat{\bfbeta}}_\hil &= \frac{1}{\left\|\bfbeta
\right\|_p}\left(\inner{\mathbf{x}}{\bfbeta}_\hil+\beta_0\right)\\
			&= \frac{1}{\left\|f^\prime\left(\mathbf{x}\right)
...
\begin{proof}
	\leavevmode
\begin{enumerate}
	\item Låt $\mathbf{x}_1$ och $\mathbf{x}_2$ vara två punkter i $L$. Då gäller att $f\left(\mathbf{x}_1\right)=f\left(\mathbf{x}_2\right)=0$ och
	\begin{align*}
